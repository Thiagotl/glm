...
Nesta seção são apresentados os resultados da elaboração dos modelos para explicar a variabilidade de produção de clorofila a. Neste modelo, a variável resposta $Y(chlorophyll_a)$ segue uma distribuição Gama. A distribuição Gama é usada para modelar variáveis contínuas positivas e assimétricas, como concentrações químicas. Seja $Y~Gamma(\mu,\phi)$ em que: $\mu$ é a média e $\phi$ é o parâmetro de dispersão. 
...
Foi elaborado um novo modelo com $AIC=317$, *Residual Deviance = 28,3*. O mesmo é apresentado na tabela \ref{tab:tab2}, observa-se que as variáveis significativas para o modelo são, *total_nitrogen*, *total_phosporus*, *dissolved_oxigen*, *pH_whater*, *carbon_dioxide_water*.
